#!/bin/bash
#
# Add a new site to Nginx running Node and Express
...
# Check arguments
if [ $# -lt 2 ]; then
    echo 'Syntax: nginx-add-node <site-name> <domain>[:<port>] [<user-name>] [<dest-dir>]'
    exit
fi
...
cp -Rn "$(dirname $0)/nginx-node-template/." $DESTDIR
...
chown $RUN_AS_USER:$SUDO_USER $DESTDIR/app.js
chmod g+w $DESTDIR/app.js
...
echo "Created Node site $1 successfully at $DESTDIR"
